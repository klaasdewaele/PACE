"""
plot.py — visualisation functions for PACE pipeline output.

Provides two main plotting functions:

  ``plot(**kwargs)``
      Per-timepoint per-sample subplot grid showing kinetic parameter traces
      across all wells relative to the MIC, with the predicted MIC highlighted.

  ``plot_overview(df, ...)``
      Summary heatmap / overview plot across all timepoints and antimycotics,
      showing how EA and CA evolve over time for the best parameter-threshold
      pair selected by evaluate.py.
"""
