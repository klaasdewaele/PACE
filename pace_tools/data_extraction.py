"""
data_extraction.py — kinetic feature extraction from time-lapse absorbance data.

Reads per-well time-lapse absorbance measurements from Excel files and derives
a comprehensive set of temporal features for each well relative to the
reference MIC well.  Derived features include:

  - Raw parameter value and normalised value at each timepoint
  - Rate of change (instantaneous and windowed mean rate)
  - Cumulative sum, cumulative rate, and cumulative mean rate
  - Normalised cumulative value (as ratio relative to first measurement)
  - Ratios of each feature between a well and its higher-concentration neighbour

The main entry point is ``extract_derive_parameters()``, which returns a nested
dictionary ``d[antimycotic][file][well][feature][timepoint]`` that is consumed
by ``predict.py``.
"""
...
def extract_parameter(d, df, parameter, file, antimycotics, window, well_names, MIC_dict, training_files):
    ...
    for antimycotic in antimycotics.keys():
        ...
        MIC_column = int(MIC_dict[file][antimycotic][1:])
        if MIC_column == 13: # If MIC is 13 (limit low) to calculate correct difference MIC_column needs to be made 12
            MIC_column = 12            
        if file in training_files: # training_files means here: files for which a reference MIC is available and where data is extracted and stored relative to MIC
            # Iterate through dataframe and define MIC and other columns
            for name, content in df_parameter_antimycotic.items():
                # df_parameter_antimycotic contains well name (e.g. 'A1') and a numpy array with extracted parameter values
                # Put these values in a dictionary where well names are expressed relative to MIC: compare name with the entry in MIC_column
                logging.debug(f'--- --- --- Extracting well {name}')
                # Extract growth control
                if int(name[1:]) == 12:
                    d[antimycotic][file]['GC'][parameter] = content.values
                    extract_rate_cum(d, antimycotic, file, f'GC', parameter, content.values, window)
                # Identify MIC column
                elif int(name[1:]) == MIC_column: # we're checking A1 to A11 - in case of A12, this has been already assigned GC - so no overlap with MIC_column 12 (set in case of limit low)
                ...
                else: # When name is not growth control and name is not MIC: calculate difference relative to MIC column
                    # e.g. column A5 will be 'MIC2' if MIC is A7. If MIC is 12 (limit low value - in MIC_dict coded as A13), column A5 will be MIC7
                    difference = MIC_column - int(name[1:])
