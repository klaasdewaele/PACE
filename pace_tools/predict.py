"""
predict.py — growth pattern detection and MIC prediction.

For each sample file, antimycotic, timepoint, kinetic parameter, and threshold,
this module:

  1. ``define_thresholds()`` — initialises the candidate threshold values for
     each parameter based on the range of observed values across wells.
  2. ``categorise()`` — applies a threshold to a single feature value and
     returns True (growth detected) or False (growth suppressed).
  3. ``predict_MIC()`` — scans the ordered list of wells for a target
     growth/no-growth pattern (default: [False, True, True, True]) and
     returns the index of the predicted MIC well together with an error flag.
  4. ``predict()`` — orchestrates the above across all files, wells,
     parameters and thresholds, populating the main ``di`` data structure
     used by ``evaluate.py`` for threshold optimisation.
"""
...
def predict_MIC(antimycotic, file, ref_wells, parameter, timepoint, threshold, di, bias, eucast, pattern, multiple_unpenalized):
    ...
    error = 'no_error'
    number_Falses_pattern = pattern.count(False) # Count Falses in pattern: it is assumed that Falses and Trues are grouped with Falses at beginning
    ...
    if all(val is None for val in bool_list):
    ...
    # growth in highest log2 dilution with a certain threshold would translate to bool_list of [None, None, True, True, ...]
    # absence of growth in lowest log2 dilution with a certain threshold: bool_list of [None, None, False, True, False, None]       
    previous_item = None # previous_item is to check value of list item previous to the one considered in the loop: must be {} or beginning of list
    ...
    for index, item in enumerate(bool_list):
        ...
        if item and (type(previous_item) != bool) and bool_list[index+1]: # At least two adjacent True values for limit-high
    ...
    # That's why we put what follows in else clause: only if NO limit high value detected, look for a limit low value
    else: # if no [None, True] or [True, ...] pattern detected: go on to detect [False, None] and [..., False] pattern
        ...
        for index, item in enumerate(bool_list[::-1]):
            ...
            # Determine the position of the last False before True's in the range of wells - this position corresponds to the position of the predicted MIC
            # One cave at: what if growth only in well 11 or 12: [..., None, None, False, True, None, None]? Pattern will not be detected
            ...
            if len(pattern_position) == 1 and not multiple_unpenalized: # Exclude EUCAST approach: there we don't differentiate multiple from single detection
                # one position where the pattern is detected
                logging.debug(f'--- --- --- --- --- Pattern detected at position {pattern_position}')
                pattern_position = [pattern_position[0] + bias + (number_Falses_pattern - 1)] # if one False in pattern, no shift, if multiple Falses, predict MIC of place of lowest/rightmost False
                logging.debug(f'--- --- --- --- --- Pattern position after bias correction {pattern_position}')
            else:
                if len(pattern_position) > 1 and not multiple_unpenalized: # when required pattern is detected multiple times: set pattern outside range of valid wells, at -1
                    pattern_position = [-1] # putting pattern position at -1 maximizes the distance to the MIC, that is at position 12
                    error = 'multiple'
                    logging.debug(f'--- --- --- --- --- Multiple matches penalized, pattern_position {pattern_position}')
                elif not pattern_position: 
                    # if pattern not found, try trimming excessive Falses from pattern so that number of Falses at start of pattern is maximal number of False in sequence in bool_list
                    max_false_seq = 0
                    current_false_seq = 0
                    for i in range(len(bool_list_extended)):
                        if bool_list_extended[i] == False:
                            current_false_seq += 1
                        elif bool_list_extended[i] == True:
                            max_false_seq = max(max_false_seq, current_false_seq)
                            current_false_seq = 0
                    Falses_to_trim = max(number_Falses_pattern - max_false_seq, 0) # Trim the number of Falses that are more in pattern than in bool_list off pattern
                    pattern = pattern[max(Falses_to_trim, 0):]
                    logging.debug(f'--- --- --- --- --- Pattern not found: falses to trim: {Falses_to_trim}, new pattern: {pattern}')

                    # Search again with trimmed pattern
                    pattern_position = [x for x in range(len(bool_list)) if bool_list_extended[x:x+len(pattern)] == pattern]
                    if len(pattern_position) == 1:
                        pattern_position = [pattern_position[0] + bias + (number_Falses_pattern - Falses_to_trim - 1)] # if one False in pattern, no shift
                        logging.debug(f'--- --- --- --- --- Pattern position after bias correction and pattern trimming {pattern_position}')
                    elif len(pattern_position) > 1 and not multiple_unpenalized:
                        pattern_position = [-1] # putting pattern position at -1 maximizes the distance to the MIC, that is at position 12
                        error = 'multiple'
                        logging.debug(f'--- --- --- --- --- Multiple matches penalized: pattern_position {pattern_position}')
                    elif len(pattern_position) > 1 and multiple_unpenalized: # If multiple matches after pattern trimming, choose rightmost
                        pattern_position = [pattern_position[-1] + bias + (number_Falses_pattern - Falses_to_trim - 1)] # if one False in pattern, no shift, if multiple Falses, predict MIC of place of lowest/rightmost False
                        logging.debug(f'--- --- --- --- --- Multiple matches unpenalized, rightmost pattern_position {pattern_position}')
                    elif not pattern_position:
                        pattern_position = [-1]
                        error = 'not_found'
                        logging.debug(f'--- --- --- --- --- Pattern not found despite trimming: pattern_position {pattern_position}')
                else: # This is case when more than one pattern match, if multiple_unpenalized (such as conventional EUCAST approach): select rightmost pattern position + apply bias correction 
                    logging.debug(f'--- --- --- --- --- Pattern detected at position {pattern_position} (multiple matches)')
                    pattern_position = [pattern_position[-1] + bias + (number_Falses_pattern - 1)]
                    logging.debug(f'--- --- --- --- --- Pattern position after bias correction {pattern_position}')
...
def define_thresholds(timepoint, antimycotic, parameters, steps, filelist, d, thresholds, eucast, threshold_dict, training):
    ...
    # for training, a random subsample of filelist will have been selected, and supplied to predict as "filelist" (after excluding files without data?)
        if training:
            ...
            logging.info(f'--- --- Read following from threshold_dict: parameter {parameter}, threshold {threshold}, bias {bias}')
            thresholds[timepoint][antimycotic][parameter] = threshold
            return thresholds, [parameter], bias
        ...
        if training:
            logging.debug(f'--- --- EUCAST threshold dictionary to be set up; thresholds is {thresholds.items()}, threshold_dict is {threshold_dict.items()}')
            for parameter in parameters: 
                thresholds[timepoint][antimycotic][parameter] = [*threshold_dict[antimycotic]] 
            return thresholds, parameters, 0
        else:
            parameter = '_'.join(threshold_dict[f'{antimycotic}_{timepoint}'].split('_')[:-2]) # format threshold_dict, e.g. {'A_46800': 'SESAfungi_ratio_-2_0.9886754950370688'}
            threshold = [float(threshold_dict[f'{antimycotic}_{timepoint}'].split('_')[-1])]
            bias = int(threshold_dict[f'{antimycotic}_{timepoint}'].split('_')[-2])
...
def categorise(antimycotic, filelist, thresholds, parameters, timepoint, di, d, eucast):
    # eucast_threshold_values is a dict that contains the threshold per file for a certain timepoint, it's only used for plotting
    eucast_threshold_values = {}
    for file in filelist:
        logging.info(f'--- --- --- Categorise {file}')
        if eucast: 
            eucast_threshold_values[file] = {} # leave empty dict if non-EUCAST, so plot() knows threshold is thresh_y and NOT specific to file
        di[antimycotic][file] = {}
        ...
        for well, values in d[antimycotic][file].items(): # e.g. well: MIC-12, value: {'BCA': array([], dtype=float64), 'BCA_rate': array([], dtype=float64)
            ...
            for parameter in parameters: # Copy parameter names to dict di
                ...
                if eucast: # only if EUCAST, populate eucast_threshold_values - must remain empty for plotting if not EUCAST
                    eucast_threshold_values[file][parameter] = {}
                for threshold in thresholds[timepoint][antimycotic][parameter]: # Iterate over all pre-defined thresholds and define them in dict di
                    ...
                    # If data is available: assign True if datapoint surpasses threshold and else False
                    ...
                    else: # EUCAST approach: bool value assigned according to proportion compared to GC - cannot be applied to ratio parameters
                        # this is only valid for parameters with min 0 and that are Normalized
                        ...
                        if values[parameter][index] > eucast_value: 
                            ...
                            pos = False
                        di[antimycotic][file][well][parameter][timepoint][threshold]['bool'] = pos
                        eucast_threshold_values[file][parameter][threshold] = eucast_value
...
def predict(timepoint, antimycotic, filelist, parameters, ref_wells, steps, thresholds, di, MIC_dict, breakpoint_dict, threshold_dict, d, species_dict, output_dir, bias, eucast, pattern, multiple_unpenalized):
    di[antimycotic] = {}
    highlight = {}
    ...
    # Categorise() also returns a dict that per file contains the EUCAST criteria set threshold - eucast_threshold_dict is empty for non-EUCAST mode  
    ...
    for file in filelist:
        ...
        for parameter in parameters:
            ...
            for threshold in di[antimycotic][file][random_well_key][parameter][timepoint].keys():
                ...
                pattern_position, error = predict_MIC(antimycotic, file, ref_wells, parameter, timepoint, threshold, di, bias, eucast, pattern, multiple_unpenalized)
                ...
                else: # pattern_position is np.NaN when all values are None, i.e. when for this timepoint file has no data, set errors and distance to np.NaN
                    distance = np.NaN 
                    VME, ME, ATU_VME, ATU_ME = np.NaN, np.NaN, np.NaN, np.NaN
    ...
    for parameter in parameters:
        for threshold in di[antimycotic][file][random_well_key][parameter][timepoint].keys():
            ...
            #plot.plot(output_path=output_dir, d=d, di=di, params=[parameter], am=[antimycotic], files=filelist,
                     #timeframe=((0, 60000)), well_legend=ref_wells, thresh_y=threshold, timepoint=timepoint, highlight=highlight,
                     #thresh_dict = eucast_threshold_dict, y_lim = (list(di[antimycotic][file][random_well_key][parameter][timepoint].keys())[0], list(di[antimycotic][file][random_well_key][parameter][timepoint].keys())[-1]),
                    #label={})
    
    return set(no_data), eucast_threshold_dict
